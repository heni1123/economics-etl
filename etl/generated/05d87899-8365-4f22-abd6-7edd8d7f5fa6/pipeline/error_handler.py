...
class ErrorHandler:
    ...
    async def retry_with_backoff(self, func: Callable[..., Any], *args: Any, max_retries: int = 3, backoff_factor: int = 1, **kwargs: Any) -> Any:
        for attempt in range(max_retries):
            try:
                ...
                self.logger.error(f"Attempt {attempt + 1} failed: {e}. Retrying in {wait_time} seconds.")
    ...
    def log_audit_trail(self, rows_extracted: int, rows_loaded: int, status: str, error_message: Optional[str] = None) -> None:
        self.logger.info(f"Audit Trail - Rows Extracted: {rows_extracted}, Rows Loaded: {rows_loaded}, Status: {status}, Error Message: {error_message or 'None'}")
