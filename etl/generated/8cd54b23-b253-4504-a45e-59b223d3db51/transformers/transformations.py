...
class DataTransformer:
    ...
    def _year(self, record: Dict) -> int:
        ...
        if not year or not isinstance(year, str):
            raise ValueError("year must be a valid string representation of an integer")
        return int(year)
    ...
    def _gdp_growth_yoy(self, record: Dict) -> float:
        gdp_usd = self._gdp_usd(record)
        gdp_usd_prev = record.get('gdp_usd_prev', 0)
        if gdp_usd_prev == 0:
            return None
        return ((gdp_usd - gdp_usd_prev) / gdp_usd_prev) * 100

    def _population_growth_yoy(self, record: Dict) -> float:
        population = self._population(record)
        population_prev = record.get('population_prev', 0)
        if population_prev == 0:
            return None
        return ((population - population_prev) / population_prev) * 100
