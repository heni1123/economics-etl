...
class DataValidator:
    ...
    async def validate_batch(self, records: List[Dict]) -> Dict[str, any]:
        total = len(records)
        valid_count = 0
        invalid_count = 0
        failed_records = []
    ...
    def validate_record(self, record: Dict) -> ValidationResult:
        ...
        if not re.match(r'^[A-Z]{3}$', record['country_code']):
            return ValidationResult(False, 'INVALID_COUNTRY_CODE', 'country_code', "country_code must be 3 uppercase letters.", 'critical')

        if not (1960 <= record['year'] <= 2030):
            return ValidationResult(False, 'INVALID_YEAR', 'year', "year must be between 1960 and 2030.", 'critical')

        if record.get('gdp_usd', 0) < 0 or record.get('gdp_usd', 0) > 200000:
            return ValidationResult(False, 'OUTLIER_GDP', 'gdp_usd', "gdp_usd must be between 0 and 200000.", 'warning')

        if record.get('population', 0) < 0:
            return ValidationResult(False, 'INVALID_POPULATION', 'population', "population cannot be negative.", 'warning')
    ...
    def _validate_br1(self, record: Dict) -> None:
        if 'gdp_usd_prev' in record and record['gdp_usd_prev'] != 0:
            record['gdp_growth_yoy'] = ((record['gdp_usd'] - record['gdp_usd_prev']) / record['gdp_usd_prev']) * 100

    def _validate_br2(self, record: Dict) -> None:
        if 'population_prev' in record and record['population_prev'] != 0:
            record['population_growth_yoy'] = ((record['population'] - record['population_prev']) / record['population_prev']) * 100
