...
- Set up alerts for failure rates exceeding 5% and response times over 2 seconds.
...
2. Restore the database to the state before the last run:
   ```sql
...
- Check the `etl_pipeline_runs` table for the new run status.
