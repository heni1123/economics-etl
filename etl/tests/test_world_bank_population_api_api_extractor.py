...
async def test_extract_error_handling(mock_http_session):
    """Test extract method with error during extraction, expect logging and empty result."""
...
async def test_fetch_page_error_handling(mock_http_session):
    """Test _fetch_page method with error during fetch, expect exception."""
...
async def test_make_request_happy_path(mock_http_session):
    ...
    mock_http_session.get = AsyncMock(return_value=AsyncMock(status=200, json=AsyncMock(return_value={"data": []})))
    extractor = WorldBankPopulationApiExtractor({"url": "http://example.com", "params": {"per_page": 10}})
    result = await extractor._make_request("http://example.com?per_page=10&page=1")
    assert result == {"data": []}

@pytest.mark.asyncio
async def test_make_request_rate_limit(mock_http_session):
    """Test _make_request method with rate limit response, expect handling."""
    mock_http_session.get = AsyncMock(side_effect=[AsyncMock(status=429), AsyncMock(status=200, json=AsyncMock(return_value={"data": []}))])
    extractor = WorldBankPopulationApiExtractor({"url": "http://example.com", "params": {"per_page": 10}})
    result = await extractor._make_request("http://example.com?per_page=10&page=1")
    assert result == {"data": []}
...
async def test_make_request_error_handling(mock_http_session):
    """Test _make_request method with error during request, expect exception."""
    mock_http_session.get = AsyncMock(side_effect=Exception("Request error"))
    extractor = WorldBankPopulationApiExtractor({"url": "http://example.com", "params": {"per_page": 10}})
    with pytest.raises(Exception):
        await extractor._make_request("http://example.com?per_page=10&page=1")
...
async def test_retry_with_backoff_happy_path(mock_http_session):
    ...
    mock_http_session.get = AsyncMock(return_value=AsyncMock(status=200, json=AsyncMock(return_value={"data": []})))
    extractor = WorldBankPopulationApiExtractor({"url": "http://example.com", "params": {"per_page": 10}})
    result = await extractor._retry_with_backoff(extractor._make_request, "http://example.com?per_page=10&page=1")
    assert result == {"data": []}

@pytest.mark.asyncio
async def test_retry_with_backoff_error_handling(mock_http_session):
    """Test _retry_with_backoff method with repeated errors, expect final exception."""
    mock_http_session.get = AsyncMock(side_effect=Exception("Request error"))
    extractor = WorldBankPopulationApiExtractor({"url": "http://example.com", "params": {"per_page": 10}})
    with pytest.raises(Exception):
        await extractor._retry_with_backoff(extractor._make_request, "http://example.com?per_page=10&page=1")
